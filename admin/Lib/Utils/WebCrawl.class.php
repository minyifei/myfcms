...
class WebCrawl
{
	...
	private function open($url)
	{
		...
		else 
		{
			...
			$this->request->referer = $url;
			$this->request->rawheaders["X_FORWARDED_FOR"] = "127.0.0.102";
			$this->request->rawheaders["Pragma"] = "no-cache";
			$this->request->fetch($url);
			if($this->request->status==200)
			{
				$this->request->results=$this->request->results;
	...
	public function getWebContent(){
		if(!$this->open($this->url)){return null;exit;}
