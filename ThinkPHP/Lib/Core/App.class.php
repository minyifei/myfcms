...
class App {
    ...
    static public function init() {
        ...
        if(C('TMPL_DETECT_THEME')) {// 自动侦测模板主题
			//自动识别模板-myfcms
	    	if(C("AUTO_THEME") && C("MYFCMS_URLTYPE")!="html"){
	    		$devices = array(
			        "android" => "android",
			        "iphone" => "(iphone|ipod|ipad)",
			        "windowsphone" => "windows phone",
			        "generic" => "(avantgo|blazer|elaine|hiptop|palm|plucker|xiino|blackberry|kindle|mobile|mmp|midp|pocket|psp|symbian|smartphone|treo|up.browser|up.link|vodafone|wap|opera mini|ucweb)"
			    );
				...
				foreach ($devices as $key => $device) {
					$mobile = (bool) preg_match("/" . strtolower($device) . "/i", strtolower($userAgent));
					if($mobile){
						$mobile_name = $key;
						break;
					}
				...
				if($mobile){
					if($mobile_name=="android" || $mobile_name=="iphone" || $mobile_name=="windowsphone"){
						cookie("think_template","touch");
					}else if($mobile_name=="generic"){
						cookie("think_template","3g");
					}
				}else{
