...
class ContentReplaceBehavior extends Behavior {
    ...
    protected function templateContentReplace($content) {
        ...
        if(is_array(C('TMPL_PARSE_STRING')) ){
			$user_replace = C('TMPL_PARSE_STRING');
			foreach ($user_replace as $key => $value) {
				$user_replace[$key] = str_replace(array_keys($replace),array_values($replace),$value);
			}
            $replace =  array_merge($replace,$user_replace);
