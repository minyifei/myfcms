				...
				<div class="list-item">
					...
					<ul>
						 <?php  $m_arctype = M("arctype");$child = $m_arctype->where("topid=$topchannelid")->select();$typeids=$topchannelid;if($child):foreach ($child as $kk => $v):$typeids .=",".$v[id];endforeach;endif;$urltype="html"; $_result=D('Archives')->relation(true)->field('body',true)->where("1=1 and typeid in($typeids)")->order("id desc")->limit("8")->select(); if ($_result): $i=0;foreach($_result as $key=>$vo):++$i;$mod = ($i % 2 ); $arctype=$vo[Arctype];$typedir=$vo[Arctype][typedir];if($urltype=="static"):$vo["arcurl"]="__APP__/$typedir/$arctype[methodname]-$vo[id].html";$vo["typeurl"]="__APP__/$arctype[classname]/$typedir-$arctype[methodname].html";elseif($urltype=="html"):$vo["arcurl"]="__APP__/archives/$arcfix$vo[id].html";$vo["typeurl"]="__APP__/category/$arctype[typedir]/$arcfix";else:$vo["arcurl"]="__APP__/index.php?m=Archives&a=$arctype[methodname]&id=$vo[id]";$vo["typeurl"]="__APP__/index.php?m=$arctype[classname]&a=$arctype[methodname]&id=$vo[typeid]";endif; ?><li>
			                 <a href="<?php echo ($vo["arcurl"]); ?>" title="<?php echo ($vo["title"]); ?>" target="_blank"><?php echo ($vo["title"]); ?></a><?php echo (date("Y-m-d",strtotime($vo["sendtime"]))); ?> 
			                </li><?php endforeach; endif;?>
					</ul>
				</div>
				  <?php $urltype="html"; $_result=M('arctype')->field('body',true)->where("1=1  and topid=$topchannelid")->order("sortrank asc")->select(); if ($_result): $i=0;foreach($_result as $key=>$childtype):++$i;$mod = ($i % 2 );$typepro=$childtype[typepro];if($urltype=="static"):$childtype["typeurl"]="__APP__/$childtype[classname]/$childtype[typedir]-$childtype[methodname].html";elseif($urltype=="html"):$childtype["typeurl"]="__APP__/category/$childtype[typedir]/$arcfix";else:$childtype["typeurl"]="__APP__/index.php?m=$childtype[classname]&a=$childtype[methodname]&id=$childtype[id]";endif;if($typepro==2):$childtype["typeurl"]="$childtype[typedir]";endif;?><div class="list-item">
						<div class="news-title">
							<h2><?php echo ($childtype["typename"]); ?></h2>
							<p>
								<a href="<?php echo ($childtype["typeurl"]); ?>" target="_blank">MORE&gt;</a>
							</p>
							<div class="clear"></div>
						</div>
						<ul>
							<?php  $m_arctype = M("arctype");$child = $m_arctype->where("topid=$childtype[id]")->select();$typeids=$childtype[id];if($child):foreach ($child as $kk => $v):$typeids .=",".$v[id];endforeach;endif;$urltype="html"; $_result=D('Archives')->relation(true)->field('body',true)->where("1=1 and typeid in($typeids)")->order("id desc")->limit("8")->select(); if ($_result): $i=0;foreach($_result as $key=>$vo):++$i;$mod = ($i % 2 ); $arctype=$vo[Arctype];$typedir=$vo[Arctype][typedir];if($urltype=="static"):$vo["arcurl"]="__APP__/$typedir/$arctype[methodname]-$vo[id].html";$vo["typeurl"]="__APP__/$arctype[classname]/$typedir-$arctype[methodname].html";elseif($urltype=="html"):$vo["arcurl"]="__APP__/archives/$arcfix$vo[id].html";$vo["typeurl"]="__APP__/category/$arctype[typedir]/$arcfix";else:$vo["arcurl"]="__APP__/index.php?m=Archives&a=$arctype[methodname]&id=$vo[id]";$vo["typeurl"]="__APP__/index.php?m=$arctype[classname]&a=$arctype[methodname]&id=$vo[typeid]";endif; ?><li>
				                 <a href="<?php echo ($vo["arcurl"]); ?>" title="<?php echo ($vo["title"]); ?>" target="_blank"><?php echo ($vo["title"]); ?></a><?php echo (date("Y-m-d",strtotime($vo["sendtime"]))); ?> 
				                </li><?php endforeach; endif;?>
						</ul>
